...
### The main ABC function for GC. This really oughta be in the R folder as a package 
###   function, but at the moment I'm too lazy to carry over all the arguments 

a10_ABC_gc <- function(part_prev_ratio) {
  ...
  if(is.vector(dx_gc_init_tot_f) & length(dx_gc_init_tot_f)==3) {
    result <- sum(
      sapply(cal_times, function(x) {
        sum(abs(rowSums(a10_output$n_diag_total_f[,,x]) - dx_gc_init_tot_f)) + 
          sum(abs(rowSums(a10_output$n_diag_total_m[,,x]) - dx_gc_init_tot_m)) 
      })
    )
  }
  if(is.matrix(dx_gc_init_tot_f) & sum(dim(dx_gc_init_tot_f)==c(3,6))==2) {
    result <- sum(
      sapply(cal_times, function(x) {
        sum(abs(rowSums(a10_output$n_diag_total_f[,,x]) - rowSums(dx_gc_init_tot_f))) + 
          sum(abs(rowSums(a10_output$n_diag_total_m[,,x]) - rowSums(dx_gc_init_tot_m))) 
      })
    )
  }
  return(result)
