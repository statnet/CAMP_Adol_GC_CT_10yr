...
calib_test_ct <- function(obj, filename_f, filename_m) {

  nsims <- nrow(obj$param)
  
  calib_test_ct_f <- calib_test_ct_m <- array(dim=c(3,12,nsims))
  ...
  for (i in 1:nsims) {
    ...
    calib_test_ct_f[,,i] <- apply(temp$n_diag_total_f, c(1,3), sum)
    if (is.vector(dx_ct_init_tot_f)) {
      calib_test_ct_f[,1,i] <- dx_ct_init_tot_f
    } else {
      calib_test_ct_f[,1,i] <- rowSums(dx_ct_init_tot_f)
    }
  
    calib_test_ct_m[,,i] <- apply(temp$n_diag_total_m, c(1,3), sum)
    if (is.vector(dx_ct_init_tot_m)) {
      calib_test_ct_m[,1,i] <- dx_ct_init_tot_m
    } else {
      calib_test_ct_m[,1,i] <- rowSums(dx_ct_init_tot_m)
    }
    
  }
  
  pdf(filename_f)
  for(i in 1:nsims) matplot(t(calib_test_ct_f[,,i]), type='b')
  dev.off()
  
  pdf(filename_m)
  for(i in 1:nsims) matplot(t(calib_test_ct_m[,,i]), type='b')
  dev.off()
