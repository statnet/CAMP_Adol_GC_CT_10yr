...
- Added in race mixing code.  Debugged.
- Set Emily on getting race mixing data from NSFG.
- Created a script file that represents the observed trend data but if all years post 2007 were like 2007, so no behavior change.  (This isn't strictly the same as just using 2007 data, because I used all years to create smooth predicted trends, then used the 2007 numbers from that smoothed model and repteated them in later years.)

Once I have the numbers from Emily, I can put them into the import file, carry them through in both the relevant scripts (masterscript and no_behav_change_script), and then continue with diagnosing the see-sawing and calibrating, and deciding what to do about them.

## Mar 27, 2019

Got race numbers form Emily, hurray.

## Apr 1, 2019

Race numbers added.  Now going to dive into why the 2007 model is so very far from calibrating given that the 2015 model is calibrated.  How could they be so far apart?

## April 2, 2019

Spent a few hours on this, building up a spreadsheet that goes step by step by step.  Made it through the end of time point 2, where it is indeed clear that the prevalence among the NBC model should indeed go up.  So next we will see why it reverses and heads the other way after another time step.  But not today.


## April 11, 2019

OK, have gone through step by step through time step 4.  Al lseems to work appropriately; no bugs.  That is good.  And there is no reversal; it just declines.  

What i also realized just now is that we are going to need to do calibration in a race-specific way.  This can't be done just through the adjustment of transmission probabilities, as those are across the board.  instead we will revert the trans probs back to the medians from the literature, and then add in an "unmeasured partner prevalence factor" for each race/sex group specifically.  Not for each age group though.

Solving the calibration is going to be more work than it was before; there should still be a unique solution (six unknowns on six parameters); but I will need to write a routine that continually loops to find the solution. Should be fun!

Next steps:
1. Add in the part_prev_ratio code (variable, formula)
2. Re-adjust the trans probs back to the literature
3. Write the caliration code.

## April 12, 2019

Note: computer keeps crashing, and that seemed to do something to the Rproject to cause it to be corrupted.  Just had to download it anew, and copy out and back in the two files that hadn't been commited.  Seemed to work well, yay git.

Adding in the part_prev_ratio code now.

## April 17, 2019

Computer still crashing, working on TS3 via macbook loaner from Brandon Marshall's lab group.

wrote the ABC, and after a few quirks it seems to be working!!!

Reset the transmission probabilities to medians and reran.

First efforts show the outcomes are highly far from the targets.

In exploring, I learned that the tolerance in the Beaumont model is in terms of numbers of standard deviations, not an absolute value.  I've worked to redo that.  Runs are better but still far from target, although I can see that they're close for time 2.  Will rewrite so that the target measure is about distance across multiple time points, not just time step 2.


## April 30, 2019

Update after a while.  Spent a lot of time getting frustrated, trying to get different kinds of models to converge.  What I have settled on, after lots of trial and error:

- convergence criterion = diagnoses at time 2
- run lots of threshholds to get really really close - this is easy to do for this convergence criterion (but not for any others)
- pick the closest of the 100 parameters sets and run the model
- take the distribution of diagnoses by age for this outcome, and reassign it as the starting distribution.
- repeat

This all works as of now.  The problem is that this still leads to the model going off to high prevalence after time step 2.  I am not sure why. but the next step is to use the excel files I built to step through why my logic is wrong that doing having the diagnoses at time 2 match those at time 1 shouldn't then lead to a stable model afterwards.....

### May 7, 2019

Oh happy day! Yesterday I finally figured out the problem - the output I was producing ws diagnoses in school, and I was comparing it to the initial total diagnoses. So of course one would need a growing epidemic for those two numbers to match, making all subsequent years bigger.  I re-wrote the code to track both diag_insch and diag_total, as well as both inc_insch and inc_total, and to then properly compare diag_total to the initial diagnoses.  Works!!!!  Interestingly, my earlier idea still holds about needing to do two rounds to get the age distribution correct - the first version is close, and then the second version is on.  Techinically it's off by a few dozen diagnoses, but given the 10s of 1000s total, this is trivial.  Yay!

### May 9, 2019
Did the actual run!  Alas, it shows increases in GC in all ethns groups with time. Just looked at inputs, and indeed the number eversex is dropping so much for B, as is mnppy, and only offset slightly by decreased condom use.  So I need to dive deep into exactly what is happening to get the observed dynamics.

Perhaps I need to document everything, and write it all up, and in the process, and in doing it all, I might find the issue myself, and/or I can prep it for Deven to look at with an independent eye.

### May 17, 2017

Happy birthday to me! I am back on my regular laptop as of yesteday, an excellent present.

Looks like I skipped an update sometime in the last week, where I updated the excel version of the 10 year tool to accurately capture all the edits I had made to the actual (R-based) 10-year tool, in terms of outputs in and out of school, and proper calibration thereof.

And now I just updated local github and delved in here, comparing the R and excel versions. I confirmed that the nbc scenario was the same in both for the first 3 years of results.  Then I put in the behavior change around eversex only.  The numbers ever having sex go down, but the predicted diagnoses go up. So therein lies a problem! I immediately saw two possible issues:

- as the number ever having sex goes down over time, the prevalence goes up because it is calcuated on the new population size
- this creates especially weird issues for 18-yo, where the population size is dramatically smaller than the 17 yo from the year before, due to aging out.

I think the solution to both may entail ensuring that the prevalence rate is calculated based on the prevous year's population first, then carried forward as a rate and multipled by the new population size. That should solve both problems, I think?  At first I thought this wouldn't require re-calibrating since it wouldn't affect the nbc model because the pop sizes are the same from year to year.  But this also matters in terms of pop sizes varying across ages as well.

Basically, we are back to the idea that, e.g. this year's 15 year olds, are next year's 16 year olds.  At one point I dove in and determined that I didn't actually need to deal with that directly in this model, largely because of how we take a weighted average of prevalence in the model.  But we still need to calculate it correctly.  And we also do care about prevalence at each age because of calcuating the size of the susceptible population.  

### July 11, 2019
Returning after two crazy months.  Home stretch.  

### July 17, 2019
Did a bunch of coding to address the carrying forward, which operates differently when eople are entering the population (only uninfected) and leaving the population (both unifected and infected). This seems to work.  Now I need to recalibrate.

### July 19, 2019
Tried to recalibrate and it didn't work.  Then I realized that calibration has two steps to it, and one needs to comment out code and then switch it back on the second round,  Then I realized this is all messy and needs to be made reproducible.  Then I created a new file (a10_gcct_entire_project.R) whose purpose was to do that.  Then I started editing the other files to work with that.  Then I got hopelessly confused, so copied over my edited files to my active/CAMP directory, and reverted.  Now I will more systematically go about recreating and documenting what I'd done before, so that I can change it correctly.

### July 22, 2019
OK, I've reconstructed the previous calibration methods.  Code is not very reproducible, in that one has to run mutliple things and go and change the comments in the code in the middle. I had done this hoping to minimize the amount of code that is written twice, but now it's a mess.  Am going to reorganize now and get the whole thing abole to be reproducedfrom start to end while runing a single file.  And clean up some naming along the way. Then I will work to update to the revised model with aging and related prevalence changes.

### July 24, 2019
OK, spent two days trying various things to get the calibration and aging to work.Aging alone didn't help, and introduced various logical challenges.  Then, I realized that there's a different issue: having prevalence out of eversex makes sense from the point of view of calculating the likelihood that someone's partner will be infected (because only those who are sexually active are possible partners.  But this leads to various artifacts -- most notably that decreasing the number of people who've ever had sex (which is what happens in the full observed trends model) causes prevalence to rise instead of fall!  I considered different ways around this, but all are complex and unsatisfactory for different reasons.  So instead I simply switched to having prevalence be out of the whole population. This is a common thing to do, and just simplified eveything.  After some time caught on bugs (specifically, dividing by the size of the total population at one point when it should have been just the school population), I finally got something that seemed to work yesterday.  And by work, I mean both that it calibrated properly and then, when running the observed behavior change model, led to declining incidence overall.  Didn't check by race or age yet. For now I'm moving onto other things for a few days, including perhaps more writing up of background and methods, then will circle back!

### July 31, 2019

Examining results by race and year. Began the file a10_gcct_anlaysis.R.  Maybe this should be rolled into entire_project eventually, but for now it's good to have it separate to explore.

Good news is that the patterns by race make sense (B and H higher % aveted than W). And so do the results by year (exponential-ish compounding).

Next is to really dive into the things that change, by proportions, and see if the whole thing passes the sniff test.

### Aug 5, 2019

Beginning the sniff test.  Creating a file called "snifftest.R" to do this.

First question: why is eversex the same for F and M?

Sniff test seems to be reasonably OK in the sense that the overall declines in eversex and number of partners are reasonably high.  And then if you combine that with a decline in prevalence with each year (as the behavioral declines kick in) then repeatedly amplified in terms of further compoounded declines, it starts to seem reasonable.

### Aug 8

In writing up the methods, I realize that recoding the year variable from year to y2k (year-2000) might actually impact results given the presence of interaction terms.  I made a quick spreadsheet to see that this is indeed true. So it may be wise to redo all models so that year is consistently defined, perhaps as (year - 2012) so that our data are centered.

Ok, redid them all.  Turns out it doesn't matter (must have made a miscalculation in my quick excel check), which is good news, because it really ought to be independent of that.  Nonetheless, adding it in and making it consistent does make the coefficients more interpretable, and I'd lik to include them in the paper.

Bug found! Eversex was the same for M and F because of a single letter typo: having the predicted values for M be drawn form the F regression.  Rerunning the whole project right now.  Thank heavens for reproducible code!!!!


### Aug 19
NME has happened.  In the meanwhile, I figured out how to sample from the regression coefficients using the var/covar matrix, and have written that all up.  I've also worked on the paper text exensively.  And I've written code to process outputs.  I've pulled both of those pieces out into separate scripts that are called from the whole_project file.

### Aug 21

Discovered that the model was really only running for 9 years of behavior change. That is fixed. Now the output all has 12 layers - layer 1 is the input data (only existing for diagnoses); layer 2 is 2007 (which is the same between the Obs and NBC scenarios), and layers 3-12 are years 2008-2017, which reflect 10 years of accumulated behavior change.

Adding in costs from Li Yan.

Spent lots of time writing more code to process outputs.  Especially across the 100 draws from the parameters.  This makes the whole thing a 4-way array, with the need to sum or average across different dimensions in diferent analyses, which gets a bit tricky.

Going to dive in and tackle chlamydia now!! 

Mostly done, but it has trouble calibrating (can only go up to Step 12 on calib2), and the part prev numbers are much further away from 1 than they are for GC.

## To Do

- Figure out why CT is behaving as it is in terms of calib
- Figure out why CT is behaving as it is in terms of male upper bound vs observer
- Finish carrying CT through the results file.
- Think harder about CIs and whether I need to redo the calibration for each one!!! 
